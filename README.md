...
| `REALYTICA_FLOW_ALLOW_HOSTS` | Hosts a flow's http or MCP node may reach inside your own network, comma-separated. Everything private is refused by default — that range includes the cloud metadata endpoint and this API itself, and a URL field plus a stored credential is otherwise an SSRF primitive. |
