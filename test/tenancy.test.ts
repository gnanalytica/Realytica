...
describe('what a role may do', () => {
  ...
  it('lets a member record but not run the workspace', () => {
    assert.equal(can('member', 'write'), true);
...
describe('matching a token to a membership', () => {
  ...
  it('claims an invite written against the email before anybody has signed in', () => {
    const rows = [member({ email: 'new@firm.in', role: 'admin' })];
    const hit = membershipFor(rows, { subject: 'sub-9', email: 'new@firm.in' });
    assert.equal(hit?.role, 'admin');
  ...
  it('does not let a fresh invite outrank an account already bound to that address', () => {
    // The attack this closes: an admin invites "asha@firm.in" as owner while
    // Asha's own account is already bound at member. Matching the invite first
    // would silently promote whoever signs in next.
    const rows = [
      member({ subject: 'sub-1', email: 'asha@firm.in', role: 'member' }),
      member({ email: 'asha@firm.in', role: 'owner' }),
    ];
    const hit = membershipFor(rows, { subject: 'sub-1', email: 'asha@firm.in' });
    assert.equal(hit?.role, 'member');
  });
...
describe('keeping a workspace administrable', () => {
  ...
  it('refuses to let the last owner step down', () => {
    const only = rows(['owner', 'admin', 'member']);
    assert.equal(wouldOrphanWorkspace(only, 't1', only[0]!, 'admin'), true);
  ...
  it('allows it once there is a second owner', () => {
    ...
    assert.equal(wouldOrphanWorkspace(two, 't1', two[0]!, 'member'), false);
  });

  it('does not care what happens to anybody who is not an owner', () => {
    const some = rows(['owner', 'admin']);
  ...
  it('counts owners in this workspace only', () => {
    ...
    assert.equal(wouldOrphanWorkspace([here, elsewhere], 't1', here, 'member'), true);
  });
});
...
describe('how a caller is written down', () => {
  it('prints the email, because a trail is read by people', () => {
    assert.equal(
      actorOf({ subject: '1148203984710', email: 'asha@firm.in', tenantId: 't1', role: 'member' }),
      'asha@firm.in',
    );
  });

  it('falls back to the subject when a provider gives no email', () => {
    assert.equal(actorOf({ subject: 'sub-1', email: '', tenantId: 't1', role: 'member' }), 'sub-1');
  });
