...
export const CHECK_FIELDS: Record<string, CheckFieldDef[]> = {
  ...
  'indicative_valuation.income_inputs': [
    ...
    { key: 'inc_gross', label: 'Gross annual income', kind: 'computed', unit: 'INR/yr', formula: { op: 'multiply', left: { op: 'multiply', left: { op: 'field', key: 'let_area' }, right: { op: 'field', key: 'achievable_rent' } }, right: { op: 'const', value: 12 } } },
    { key: 'inc_after_vacancy', label: 'Less vacancy', kind: 'computed', unit: 'INR/yr', formula: { op: 'multiply', left: { op: 'field', key: 'inc_gross' }, right: { op: 'subtract', left: { op: 'const', value: 1 }, right: { op: 'divide', left: { op: 'field', key: 'vacancy_pct' }, right: { op: 'const', value: 100 } } } } },
    { key: 'inc_noi', label: 'Net operating income', kind: 'computed', unit: 'INR/yr', formula: { op: 'multiply', left: { op: 'field', key: 'inc_after_vacancy' }, right: { op: 'subtract', left: { op: 'const', value: 1 }, right: { op: 'divide', left: { op: 'field', key: 'opex_pct' }, right: { op: 'const', value: 100 } } } } },
    { key: 'inc_capitalised', label: 'Capitalised value', kind: 'computed', unit: 'INR', formula: { op: 'divide', left: { op: 'field', key: 'inc_noi' }, right: { op: 'divide', left: { op: 'field', key: 'cap_rate_pct' }, right: { op: 'const', value: 100 } } } },
  ],
  'indicative_valuation.residual_inputs': [
    ...
    { key: 'finance_pct', label: 'Finance rate', kind: 'percent', unit: '% per year', hint: 'An ANNUAL rate. Charged on the average outstanding balance over the build period, not on the whole cost for the whole time — see the working on the Value tab.' },
    ...
    { key: 'build_months', label: 'Build period', kind: 'duration', unit: 'months', required: false, hint: 'Drives the finance charge and the discount back to today. Without it finance is charged flat and the residual is not discounted.' },
    { key: 'discount_rate_pct', label: 'Discount rate', kind: 'percent', unit: '% per year', required: false, hint: 'Brings the residual back from completion to today. A site worth ₹10 Cr in three years is not worth ₹10 Cr now.' },
    { key: 'land_acquisition_pct', label: 'Land acquisition costs', kind: 'percent', unit: '% of land value', required: false, hint: 'Stamp duty, registration and legal on buying the site. Karnataka is around 5.6% + 1% registration.' },
    { key: 'resid_fees', label: 'Fees on cost', kind: 'computed', unit: 'INR', formula: { op: 'multiply', left: { op: 'field', key: 'construction_cost' }, right: { op: 'divide', left: { op: 'field', key: 'professional_fees_pct' }, right: { op: 'const', value: 100 } } } },
    { key: 'resid_finance', label: 'Finance charge', kind: 'computed', unit: 'INR', formula: { op: 'multiply', left: { op: 'multiply', left: { op: 'multiply', left: { op: 'field', key: 'construction_cost' }, right: { op: 'divide', left: { op: 'field', key: 'finance_pct' }, right: { op: 'const', value: 100 } } }, right: { op: 'divide', left: { op: 'field', key: 'build_months' }, right: { op: 'const', value: 12 } } }, right: { op: 'const', value: 0.5 } } },
    { key: 'resid_marketing', label: 'Marketing on GDV', kind: 'computed', unit: 'INR', formula: { op: 'multiply', left: { op: 'field', key: 'gdv' }, right: { op: 'divide', left: { op: 'field', key: 'marketing_pct' }, right: { op: 'const', value: 100 } } } },
    { key: 'resid_profit', label: 'Profit on GDV', kind: 'computed', unit: 'INR', formula: { op: 'multiply', left: { op: 'field', key: 'gdv' }, right: { op: 'divide', left: { op: 'field', key: 'developer_profit_pct' }, right: { op: 'const', value: 100 } } } },
    { key: 'resid_total_cost', label: 'Total development cost', kind: 'computed', unit: 'INR', formula: { op: 'add', left: { op: 'add', left: { op: 'add', left: { op: 'add', left: { op: 'field', key: 'construction_cost' }, right: { op: 'field', key: 'resid_fees' } }, right: { op: 'field', key: 'resid_finance' } }, right: { op: 'field', key: 'resid_marketing' } }, right: { op: 'field', key: 'resid_profit' } } },
    { key: 'resid_at_completion', label: 'Residual at completion', kind: 'computed', unit: 'INR', formula: { op: 'subtract', left: { op: 'field', key: 'gdv' }, right: { op: 'field', key: 'resid_total_cost' } } },
    { key: 'resid_discounted', label: 'Discounted to today', kind: 'computed', unit: 'INR', formula: { op: 'divide', left: { op: 'field', key: 'resid_at_completion' }, right: { op: 'power', left: { op: 'add', left: { op: 'const', value: 1 }, right: { op: 'divide', left: { op: 'field', key: 'discount_rate_pct' }, right: { op: 'const', value: 100 } } }, right: { op: 'divide', left: { op: 'field', key: 'build_months' }, right: { op: 'const', value: 12 } } } } },
    { key: 'resid_land_value', label: 'Land value after acquisition costs', kind: 'computed', unit: 'INR', formula: { op: 'divide', left: { op: 'field', key: 'resid_discounted' }, right: { op: 'add', left: { op: 'const', value: 1 }, right: { op: 'divide', left: { op: 'field', key: 'land_acquisition_pct' }, right: { op: 'const', value: 100 } } } } },
  ],
