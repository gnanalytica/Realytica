/**
 * Fixture cases for the test suite and the eval harness.
 *
 * These were the demo data until the demo became a set of Bengaluru cases a
 * person is meant to look at (`seed.ts`). Splitting them was not tidying: the
 * two have opposite requirements. Demo data should change whenever a better
 * story is available; fixtures must NOT, because roughly a dozen test files
 * assert screen outcomes against these exact identities — a gram panchayat
 * site that screens badly, a BDA site that screens well, an Amsterdam
 * apartment that exercises the Netherlands pack. While they were one array,
 * improving the demo meant rewriting test expectations, which is how a
 * regression net quietly stops being one.
 *
 * Deliberately covers both country packs and both ends of the outcome
 * spectrum, in each currency where practical. Change these only to test
 * something new, never to make a demo look better.
 */
...
/**
 * Realistic filenames materialised as `CaseDocument`s for each fixture case,
 * keyed by the case's `identity.label`. Filenames are chosen to exercise
 * `classifyDocument`'s keyword patterns correctly.
 */
export const FIXTURE_DOCUMENT_FILENAMES: Record<string, string[]> = {
